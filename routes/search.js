...
router.get('/:query/:page?', function(req, res, next) {
  ...
  function (error, response) {
	if(error){
		...
		res.send(JSON.stringify({
			code: 0,
			response: 'Our search server appears to be offline. If you could please report this to <a href="https://twitter.com/irukandjilabs"><strong>@irukandjilabs</strong></a> it would be awesome!'
		}));
		return;
	}
